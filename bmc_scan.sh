...
VENDOR[3]="Supermicro,ADMIN,ADMIN,00:25:90"
VENDOR[4]="IBM_X,admin,passw0rd,98:BE:94"
VENDOR[5]="Lenovo,USERID,PASSW0RD, 7C:D3:0A"
    ...
    for (( l = 0; l < ${#VENDOR[@]}; l++ )); do
      for k in `nmap --system-dns -sP $j | grep -B2 -i "$(echo ${VENDOR[$l]}| cut -d',' -f4)" | grep Nmap | awk '{print $5}'`; do
        ...
        if [ ! -z "${INFO}" ]; then
          ...
          printf "%02d %-10s %-9s  %-15s  %s  %s\n" $COUNT $(echo ${VENDOR[$l]} | cut -d',' -f1) $SN $INFO "$PN" 2>&1 | tee -a ~/.bmclist
        fi
      done
    done
  done
